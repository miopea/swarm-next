...
- Browser transport starts only after the terminal host element is mounted,
  fonts are ready, and xterm can propose real dimensions during a bounded
  post-layout fit window. An eager fit in the mount call is insufficient
  because xterm can otherwise retain its 80-by-24 default inside a much larger
  stable container.
