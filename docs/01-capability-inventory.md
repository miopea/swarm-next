...
| Jira | Redesign | Local dogfood slice implemented: operator OAuth, project pools, explicit workflow mapping and selective intake, linked task identity, safe reconciliation, and durable bounded outbound transitions with visible conflict/retry. Jira-backed Apiary groundwork now includes durable invitations, server-derived fail-closed join readiness, a Keeper-owned promoted-project catalog, an atomic private promotion command and Keeper UI, revision-bound operator acceptance, audited sole-Keeper collapse, a durable Ed25519 node identity with short-lived signed Hive connection-card download, Keeper-side verification/pinning of independent Hive identities without membership, and signed one-time invitation issuance with digest-only secret storage. Invited-Hive import/consumption, cross-Hive catalog propagation and acknowledgement, and atomic shared claims remain staged work. |
