...
| BFG Admin shared support | Build | ADR 0080: one central email-contact support source for all Hives, separate from Admin-approved development-task intake. Initial validation and isolated atomic storage are in progress; public ingress, Hive outbox, Admin conversation reads, attachments and approved original-channel responses are not activated. Task closure returns linked completion evidence, never customer-send permission. |
