...
| BFG Admin shared support | Build | ADR 0080 corrected ownership: BFG Admin owns central Messages, Requests and customer replies in its existing database; no separate Swarm support server. The Hive client has a bounded private outbox and authenticated submission/retry UI. Actual Admin-route restart/exact-replay tests pass; Admin separately reports its approved mail/attachment roundtrip passed. Development-Hive activation is approved but live submission/recovery UI verification remains pending. Native attachment upload and the full linked task-to-approved-reply journey remain open. Task closure is never customer-send permission; MCP workspace binding is a separate approval. |
