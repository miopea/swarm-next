...
| BFG Admin shared support | Build | ADR 0080: one central email-contact support source for all Hives, separate from Admin-approved development-task intake. Isolated atomic intake and credential-protected Admin list/thread reads pass fictional tests and initial Admin schema conformance; not publicly activated. Hive outbox, diagnostics/attachments, inbound email and approved original-channel replies remain unfinished. Task closure returns linked completion evidence, never customer-send permission. |
