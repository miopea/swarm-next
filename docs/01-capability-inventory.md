...
| Direct terminal input | Keep | Essential, with explicit input ownership, stale-session protection, provider-prompt guards, and content-free actor/shape attribution at the terminal-host write boundary. Typed content and terminal output never enter that audit. Maturity ADR 0062 adds a tested generation-bound domain model, engine effect guard, protocol-11 commands, and v4 API adapter with ordered activity projection, and browser Resume Here control. Local disposable-PTY and persistence checks cover the foundations; browser refusal of legacy grants, passive-view input/resize gates, and async-fit ownership guards are locally tested. Full device and rolling-update acceptance remain pending. |
...
| Completion verification | Redesign | Supported routine evidence is settled by the coordinator without mandatory Queen approval; exceptions retain Queen/operator judgment. Release evidence remains required when shipping is part of done. Live lifecycle acceptance remains open. |
| Reviewed-work exchange | Redesign | Hand-back preserves Review and atomically queues the request. ADR 0066 adds exact request-linked answers, returning next-move ownership to Queen without treating progress messages as answers or completing the task. Live provider acceptance remains open. |
| Queen task assignment | Redesign | Preserve assisted coordination but clarify recommendation, authorization, and execution. ADR 0081 adds an in-place, revision-checked blocker reassessment command: Queen can record a verified current reason and external time window without waking workers or changing task state. Verified dependencies remain explicit prerequisite links. Fleet-level recovery and evidence-backed no-action coverage still require live acceptance. |
...
| Questions before deciding | Redesign | Implemented as a bounded, non-authorizing exchange attached to the exact Needs You decision. Asking preserves original choices and permission gates; replies identify the actual requester or Queen, return operator attention, and survive refresh. Native Queen and sleeping-worker exchanges passed in an isolated Hive; deployment and real-device status are tracked in doc 94. Direct terminal/AskUser answer correlation remains a separate requirement. |
| Proposal approval system | Merge | One generalized operator-decision inbox rather than feature-specific proposal surfaces. Obsolete pending requests can be withdrawn by their requester or Queen without recording an operator answer or grant (ADR 0074); quiet history preserves actor, reason and time. Live Queen recovery acceptance remains open. |
...
Ops Console ticket intake is an approved addition under ADR 0072, currently in
...
| Inter-worker messages | Redesign | Queen-mediated task messages use the bounded durable outbox in ADR 0067: exact claims, no automatic replay of uncertain writes, explicit Queen reconciliation, and cancellation of obsolete queued review questions without erasing history. Ordinary Queen questions are transactionally limited to the task's current assignee; the sole cross-worker exception is an explicit second opinion from an awake, unengaged, active-task-free managed Scout, with availability rechecked before terminal input. Queues owns delivery exceptions, not timer-driven Needs You. Live provider and crash/rolling-update acceptance remain open. |
...
| Cross-project tasks | Redesign | Queen owns cross-worker routing; ordinary workers stay in their lane. ADR 0076 adds bounded explicit same-Hive task prerequisites through shared operator/Queen commands, guarded transitions and automatic delivery/start admission. Queues and task cards show current upstream state and owner without inferring dependencies from prose or queue order. Implementation is under local verification; isolated live Queen coordination and operator dependency editing remain acceptance work. |
...
| BFG Admin shared support | Build | ADR 0080 corrected ownership: BFG Admin owns central Messages, Requests and customer replies in its existing database; no separate Swarm support server. The Hive client has a bounded private outbox and authenticated text submission/retry UI, with live text intake/reload acceptance in doc 86. Native attachments now have atomic immutable file storage, multipart retry transport, authenticated ingress and reviewed browser-copy recovery, verified in isolation; paired Admin acceptance and deployment remain pending. The full linked task-to-approved-reply journey remains open. Task closure is never customer-send permission; MCP workspace binding is a separate approval. |
...
| Operator availability | Redesign | At Hive, Reachable, and scheduled/manual Night Watch are wired to the existing presence and autonomy policies. Automatic At Hive requires fresh desktop activity; mobile use remains Reachable and cannot end Night Watch. Reachable preserves installed `away` wire/storage preferences (ADR 0018). Real-device lock detection, observation ordering, and notification delivery acceptance remain pending. |
...
| Capability | Decision | Rationale and intended direction |
|---|---|---|
| SQLite persistence | Keep/Redesign | Embedded source of truth with one owner, transactional migrations, backups, and integrity checks. The latest migration and declared schema ceiling share one named version, and a structural immediately-previous-schema test prevents an unreachable compiled step. Schema 136 also repairs the integrated schema-124 identity collision so a database already stamped by the former Ops migration cannot remain without terminal-control projection storage. ADR 0075 adds a shared in-process containment latch after confirmed runtime integrity damage, an owned opportunistic hourly probe, and database-independent health/attention reporting; verified offline recovery remains explicit. |
