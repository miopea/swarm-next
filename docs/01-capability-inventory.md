...
| Legacy migration | Redesign | Preview-first, versioned migration packages keep Legacy read-only during import. The first implemented slice exports open Legacy task records without paths or credentials, excludes Jira and closed work, normalizes worker ownership and lifecycle, imports an explicit selection atomically with provenance, starts no workers, and allows rollback only while the batch is untouched. A separate backed-up receipt finalization will leave transferred Legacy tasks visible but read-only rather than completed; no dual write is allowed. |
