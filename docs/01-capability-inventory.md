...
| Capability | Decision | Rationale and intended direction |
|---|---|---|
| SQLite persistence | Keep/Redesign | Embedded source of truth with one owner, transactional migrations, backups, and integrity checks. The latest migration and declared schema ceiling share one named version, and a structural immediately-previous-schema test prevents an unreachable compiled step. Schema 136 also repairs the integrated schema-124 identity collision so a database already stamped by the former Ops migration cannot remain without terminal-control projection storage. |
