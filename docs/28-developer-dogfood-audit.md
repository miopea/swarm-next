...
- Release `0.1.0-35d144421675` preserved terminal-host PID `1832378` and the attached Queen conversation while updating App/API. A bounded Queen review rendered its long prompt as a new Claude `[Pasted text #N]` chip, Swarm submitted it automatically, Queen processed the review, and the exact automation run closed as `needs_operator` without a manual Enter. An unrelated operator-owned draft remained untouched in Queen's prompt.
