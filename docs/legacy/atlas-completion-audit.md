...
| Bring only genuine product choices to the operator | `ring1-legacy-decision-packet.md` removes mechanical fixes and records seven evidence-backed operator choices against current Next boundaries, including transparent developer-owned learning, Queen-discovered Routines, and the worker queue as the sole preparation surface. | **Active and recorded through 2026-08-17.** | Continue the focused opportunity interview; reopen a recorded choice only if bounded Ring 1 evidence meets its stated change condition. |
