...
| Bring only genuine product choices to the operator | `ring1-legacy-decision-packet.md` removes mechanical fixes and records ten evidence-backed operator choices against current Next boundaries, including transparent learning, Queen-discovered Routines, worker queues, conversation renewal, Return briefings, and independently earned provider support. | **Active and recorded through 2026-08-17.** | Continue the focused opportunity interview; reopen a recorded choice only if bounded Ring 1 evidence meets its stated change condition. |
