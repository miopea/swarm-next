...
| Bring only genuine product choices to the operator | `ring1-legacy-decision-packet.md` removes mechanical fixes and records eight evidence-backed operator choices against current Next boundaries, including transparent developer-owned learning, Queen-discovered Routines, worker queues as the preparation surface, and explainable conversation renewal. | **Active and recorded through 2026-08-17.** | Continue the focused opportunity interview; reopen a recorded choice only if bounded Ring 1 evidence meets its stated change condition. |
