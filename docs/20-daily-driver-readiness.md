...
| Work from Jira without opening Jira | Browser-bound task lookup and ad hoc synchronization | Assigned open work can synchronize every minute, unassigned work is explicitly claimed, linked issues include terminal states, comments and mapped workflow changes use durable bounded outboxes, remote closure or reopen is canonical, and concurrent remote status changes become visible review-before-retry conflicts | Ready for bounded Ring 1 use; keep real Jira mutations operator-initiated during early dogfood |
