...
mod tests {
    ...
    fn schema_125_migrates_and_reopen_preserves_evidence() {
        ...
        let path = directory.path().join("evidence.sqlite");
        {
            ...
            connection
                .execute_batch("ALTER TABLE task_dispatches DROP COLUMN generation; DROP TABLE worker_startup_context; DROP TABLE browser_evidence; PRAGMA user_version = 125;")
                .unwrap();
        }
        {
            let store = TaskStore::open(&path).unwrap();
