...
mod tests {
    ...
    fn selection_migration_preserves_prior_manual_override() {
        ...
        store
            .repoint_provider_conversation(worker.id, &manual)
            .unwrap();
        store.connection().unwrap().execute_batch("DROP TABLE operator_statement_resolutions; DROP TABLE operator_statements; ALTER TABLE task_dispatches DROP COLUMN generation; ALTER TABLE worker_startup_context DROP COLUMN selection_revision; ALTER TABLE worker_startup_context DROP COLUMN selection_suspended; PRAGMA user_version = 127;").unwrap();
        drop(store);
        let store = TaskStore::open(&path).unwrap();
        assert!(
            !store
    ...
    fn migration_does_not_guess_context_for_existing_sessions() {
        ...
        store
            .connection()
            .unwrap()
            .execute_batch("DROP TABLE operator_statement_resolutions; DROP TABLE operator_statements; ALTER TABLE task_dispatches DROP COLUMN generation; DROP TABLE worker_startup_context; PRAGMA user_version = 126;")
            .unwrap();
        drop(store);
        let store = TaskStore::open(&path).unwrap();
        assert_eq!(
