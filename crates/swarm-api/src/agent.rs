...
pub(super) const QUEEN_BLOCK_RECOVERY_GUIDANCE: &str = "BLOCK RECOVERY. Read blocked_reassessment in swarm_list_coordination_attention. These tasks have no pending linked operator decision, unfinished explicit prerequisite, or future recorded hold; that is a reason to investigate, NOT proof that their blocker cleared. Read current task history and decisions, and ask the assigned worker when evidence is missing. When the blocker is verified cleared, use swarm_transition_task to move Blocked to Ready, preserving ownership and current active work; assign an unowned task to its proper worker. Capacity or your routing backlog is not a blocker. If another task is still required, record its verified prerequisite link; do not invent links from prose. For a verified future calendar window or dated operator instruction, use swarm_reassess_task_block with the latest activity sequence, checked evidence and not_before. A date written only in prose is not a scheduled hold. Preserve the source's time zone; do not invent a date or use a cooldown to hide work. When the date arrives Queen must recheck the gate; it does not authorize automatic resumption. Preserve explicit operator deferrals and external resource constraints. If recovery genuinely needs operator judgment, create one task-linked Needs You decision with the blocker, worker view and your concise recommendation. If several tasks genuinely wait on the same pending decision, read each task's current review evidence and use swarm_set_task_decision_link to record additional blockers. Keep one Needs You question; do not broaden command authorization. An answered or withdrawn decision is not a current gate: reassess each task's remaining blocker instead of keeping it parked on the old request. Do not leave an unfiled operator question buried in a block note or escalate solely because time passed.";
...
pub(super) const QUEEN_REVIEW_COVERAGE_GUIDANCE: &str = "REVIEW COVERAGE. Before finishing, account for every current Queen-owned task AND worker recovery obligation in coordination attention. Route actionable work with lifecycle tools and record real dependencies or decisions structurally. For Queen-owned waits, use swarm_read_review_evidence and swarm_record_review_disposition. For a checked worker recovery wait, use its exact current recovery_identity with swarm_record_recovery_assessment. The server checks current working, protected input and pending delivery automatically; do not add approval round trips. A delivered message does not prove execution resumed. External conditions require a fresh check and actual evidence/source each run; an operator deferral requires an authenticated task-linked statement or resolved decision, not recollection or a worker's claim. A summary saying unchanged supplies no coverage. Uncovered runs finish as incomplete, not success. If you cannot finish, use incomplete rather than repeated finish calls; recovery remains yours unless a concrete issue genuinely requires the operator.";

pub(super) const QUEEN_ACTIVE_RECOVERY_GUIDANCE: &str = "ACTIVE WORK RECOVERY. Check active_work_recovery before concluding that worker-owned work is progressing. An Active task with a currently resting terminal can need your help even though its next move belongs to the worker. A queued successor waiting behind that task is correct; leaving the earlier task stranded is not. Urgency orders your attention but is NOT a recovery requirement: low-priority work and tasks with no urgent successor must also progress. A recently restarted worker or an empty prompt does not prove that its previous task resumed; check actual progress rather than assuming it is between turns. Current terminal_observation supersedes an old terminal-unreadable explanation only for terminal visibility, not task completion or authority. Read the active task history, latest worker answer and existing message delivery first. If unfinished work can safely continue within its existing scope, use one concrete swarm_message_worker request to its current assignee to continue that SAME task and conversation or report its exact blocker. That guarded path rechecks session, task ownership, prompt and operator engagement; do not type directly, bypass a hold, change provider, restart context, or transition Active through Ready. Do not repeat a pending request or send generic kicks every review; assess the prior attempt first. Preserve verified external waits and operator deferrals. If safe recovery truly fails, create a concise task-linked operator decision with what you tried and what is needed.";
                        ...
                        .and_then(|attention| {
                            ...
                            structured(json!({
                                ...
                                "queue_snapshot": {
                                    ...
                                    "next_action": "The review list puts never-assessed work first, then the oldest recorded assessment. This is review fairness, not execution priority or proof a prior wait is still valid. Handle urgent new work, but also advance this unchecked backlog each run instead of repeatedly covering the same recent cluster and leaving it incomplete. Triage Draft tasks and account for every Queen-owned item before saying nothing waits on you. Read current task facts/history before routing, deferring or settling. Preserve operator-reserved drafts by recording authenticated deferrals, not by starting them. A prose explanation does not change recorded ownership. When details are truncated, use swarm_list_tasks for remaining items. Counts and ordering never authorize clearing a blocker."
                                },
...
impl AgentMcp {
    ...
    fn read_task_history(&self, arguments: Value) -> Result<CallToolResult, ApplicationError> {
        ...
        structured(json!({
            ...
            "messages_note": "The Queen-worker exchange on this task, oldest first. A message is not a state change. Read delivery_state, delivered_at and delivered_session_id together. A queued message waits for guarded delivery; uncertain, rejected, cancelled and resolved are different outcomes, not ordinary pending work. reached_the_current_session only says whether the recorded delivery session remains open. False on a delivered message does NOT prove it was unread: a resumed provider conversation may retain it and work may already have happened. Never resend solely because the session ended, and never treat delivered as proof of comprehension, progress or an answered question. Verify the latest worker answer, task evidence and current terminal observation before deciding what continuation is needed. A null delivery session on older records is missing historical evidence, not permission to replay.",
...
fn task_decision_link_tool() -> Tool {
    tool(
        "swarm_set_task_decision_link",
        "Queen only: explicitly link another unfinished local task to a pending operator decision or an authenticated resolved ruling on an originating task, or remove an additional link. Read swarm_read_review_evidence immediately before changing links and pass its evidence_revision. For a pending answer genuinely gating several tasks, keep one question. For a resolved ruling, read its exact original wording and answer, explain why its ORIGINAL scope applies to this task, and ask the operator if applicability is uncertain. A resolved reference creates no pending gate, new permission or answer delivery; it may support a deferral after rereading review evidence. Links never change task lifecycle, assignment, command permission or delivery destination. An answer is delivered only through the original decision. Removing a link never resumes or completes work. Never infer shared authorization or link withdrawn decisions. Exact replay is safe; stale evidence requires rereading. At most 32 additional links per decision or task and 4096 per Hive.",
        &json!({"type":"object", "properties": {
            "task_id":{"type":"string","format":"uuid"},
...
fn transition_task_tool() -> Tool {
    ...
    tool(
        ...
        DESCRIPTION.get_or_init(|| format!(
            "Move a task through its explicit lifecycle. Workers may report only Active, Blocked, or Review for their own assignment. Queen must wake an assigned sleeping worker and observe its live session before moving Ready or Blocked work to Active. Include a concise Blocked reason or Review handoff note. Completed requires verification evidence, including release or handoff evidence when shipping was part of done. Awaiting release is for work you have ACCEPTED that is finished and merely unshipped — it needs no evidence to enter, and it completes ITSELF when a WHOLE-TASK deployment is recorded, so send work there rather than holding it in review or closing it on a nothing-to-deploy claim that is really ships-later; it is a resting state and not a trap, and work that turns out unfinished returns through Active. SHIPPING ONE INCREMENT OF SEVERAL DOES NOT CLOSE IT: record that with delivers_whole_task=false and the evidence is kept while the task stays exactly where it is, however many deployments it already carries. So a multi-line DONE WHEN belongs here rather than in Blocked -- unmet acceptance lines are unfinished work, not an external dependency, and parking them as a blocker hides real remaining work among genuine waits. Abandoned closes work that was superseded or given up on and asks for no evidence, because nothing shipped and nothing is coming; it is Queen's to set, like Completed. THE MOVES THAT EXIST, and every move not listed here is refused with \"task cannot move from X to Y\": {}. Read that as the exits, not only the entries: a state's meaning does not tell you what you can do next, and reading only entry conditions is how a task got parked behind a door somebody then could not find the handle for. Completed and Abandoned are the only genuinely final states. Refusal is about the SHAPE of the move and never about your authority — being told a move does not exist is different from being told it is not yours to make, and the second says so.",
            lifecycle_moves(),
        )).as_str(),
        &json!({
            "type": "object",
...
mod tests {
    ...
    fn queen_is_briefed_on_what_she_owns_and_on_capabilities_that_are_not_tools() {
        ...
        assert!(brief.contains("swarm_reassess_task_block"));
        assert!(brief.contains("checked evidence and not_before"));
        assert!(brief.contains("do not invent a date"));
        assert!(brief.contains("does not authorize automatic resumption"));
        assert!(brief.contains("swarm_set_task_decision_link"));
