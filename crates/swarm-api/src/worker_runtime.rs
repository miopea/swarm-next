...
///
/// ⚠️ THIS EXISTS BECAUSE THE FOUR CASES WERE FLATTENED INTO PROSE ONE LINE
...
///
/// Two of these are ordinary and two are faults, and the difference is what a
/// person can do about it. A worker that has never run in its workspace has no
...
/// filesystem fault, and it is the one case in this bucket where something IS
/// wrong and someone CAN fix it.
///
...
impl UnknownCause {
    ...
    pub(crate) const fn is_fault(self) -> bool {
        match self {
            Self::NeverRun | Self::NoTranscripts => false,
...
pub(crate) fn conversation_freshness(
    ...
    // could not read one of them". Without it both end at the same give-up
    // below, and a file-permission fault reads as a worker that never ran.
...
mod tests {
    ...
    fn a_directory_nobody_can_read_is_a_fault_and_a_worker_that_never_ran_is_not() {
        ...
        let benign = conversation_freshness(&profile, &projects, home.path());
        ...
        let fault = conversation_freshness(&profile, &projects, home.path());
        {
