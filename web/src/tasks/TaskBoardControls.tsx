...
export default function TaskBoardControls({ query, filter, source, sort, project, worker, workers, projects, openCount, busy = false, onQueryChange, onFilterChange, onSourceChange, onSortChange, onProjectChange, onWorkerChange, onSync }: Props) {
  ...
  return (
    <div className="task-board-controls">
      ...
      <label><span>Sort by</span><select value={sort} onChange={(event) => onSortChange(event.target.value as TaskBoardSort)}><option value="queue">Queue order</option><option value="priority">Priority</option><option value="status">Needs attention</option><option value="created">Oldest first</option><option value="updated">Recently updated</option><option value="worker">Worker</option><option value="project">Jira project</option></select></label>
