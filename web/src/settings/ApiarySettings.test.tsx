...
test("Keeper creates one private invitation link for an outbound member connection", async () => {
  ...
  expect(screen.getByText(/As Keeper, first review Return to a personal Hive/)).toBeVisible();
  expect(screen.queryByText(/Leaving is below/)).not.toBeInTheDocument();
  ...
  expect(screen.getByRole("group", { name: "Created Apiary link" })).toHaveTextContent(/paste the complete link into Settings.*Connections.*Your Apiary.*Join a Keeper's Apiary/i);
