...
export function buildSanitizedDiagnosticReport({ context, health, hiveIdentity, liveFeedState, recentEvents, runtime, sessions, workers, jiraReadiness, jiraUnavailable }: DiagnosticReportInput) {
  ...
  return {
    ...
    runtime_resources: runtime.resources ? {
      ...
      storage: runtime.resources.storage?.map(({ scope, total_bytes, available_bytes, pressure, advisory_available_bytes, critical_available_bytes }) => ({
        scope, total_bytes, available_bytes, pressure, advisory_available_bytes, critical_available_bytes,
      })),
