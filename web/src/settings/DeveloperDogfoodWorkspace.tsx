...
export default function DeveloperDogfoodWorkspace({ runtime, version, reachable, collection, operatorToken }: {
  ...
  return <section id="settings-dogfood" className="settings-card" aria-labelledby="dogfood-heading">
    ...
    {coldRestores.slowest_fit && <p>Setup breakdown for that return: {Math.round(coldRestores.slowest_fit.opening_ms)} ms opening · {Math.round(coldRestores.slowest_fit.font_ms)} ms font readiness · {Math.round(coldRestores.slowest_fit.layout_ms)} ms layout and initial sizing.</p>}
    {coldRestores.slowest_fit?.frame_count !== undefined && <p>Sizing frames for that return: {coldRestores.slowest_fit.frame_count} · longest interval {Math.round(coldRestores.slowest_fit.max_frame_gap_ms!)} ms. Intervals include browser scheduling and preceding measurement work, not CPU time.</p>}
    <small>These phases belong to the same slowest return, not independent maxima or p95 phases. Layout includes animation-frame waits, measurement and initial sizing; a delayed frame does not by itself prove CPU pressure. Font readiness includes fallback after a load failure. Connection includes retries and state application. Neither proves screen paint or input ownership. Missing or interrupted phase evidence is unavailable, not zero.</small>
