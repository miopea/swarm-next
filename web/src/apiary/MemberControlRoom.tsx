...
const emptySnapshot: MemberSnapshot = { members: [], sharedWork: [] };
...
export default function MemberControlRoom({ identity, operatorToken, onManage }: Props) {
  ...
  const refresh = useCallback(async () => {
    setState("loading");
    const [members, sharedWork, sync, catalog] = await Promise.allSettled([
    ...
    setState([members, sharedWork, sync, catalog].some((result) => result.status === "rejected") ? "partial" : "ready");
  }, [operatorToken]);
  ...
  return (
    <section className="keeper-control-room member-control-room" aria-labelledby="member-control-heading">
      <header className="keeper-hero">
        ...
        <div><p className="eyebrow">Member Hive</p><h3 id="member-control-heading">{context.apiary.name}</h3><p>Your Hive stays independently operated while the Keeper coordinates shared Jira work.</p></div>
      ...
      <dl className="keeper-summary member-summary" aria-label="Member Apiary summary">
        ...
        <div><dt>My shared work</dt><dd>{localClaims.length}</dd></div>
      </dl>
