...
export default function ApiaryWorkspace({ identity, operatorToken, busy, onIdentityChange, onOpenTasks }: {
  ...
  return context.local_role === "keeper"
    ? <KeeperControlRoom identity={identity} operatorToken={operatorToken} onManage={() => setManaging(true)} onOpenTasks={onOpenTasks} />
    : <MemberControlRoom identity={identity} operatorToken={operatorToken} onManage={() => setManaging(true)} onOpenTasks={onOpenTasks} />;
}
