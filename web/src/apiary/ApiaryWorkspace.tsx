...
export default function ApiaryWorkspace({ identity, operatorToken, busy, onIdentityChange, onOpenTasks }: {
  ...
  const [managing, setManaging] = useState<false | "settings" | "invitations">(false);
  ...
  if (managing || context?.mode !== "federated") return <div className="apiary-management">
    ...
    <ApiarySettings busy={busy} hiveIdentity={identity} operatorToken={operatorToken} initialFocus={managing === "invitations" ? "invitations" : undefined}
  ...
  return context.local_role === "keeper"
    ? <KeeperControlRoom identity={identity} operatorToken={operatorToken} onManage={() => setManaging("settings")} onInvite={() => setManaging("invitations")} onOpenTasks={onOpenTasks} />
    : <MemberControlRoom identity={identity} operatorToken={operatorToken} onManage={() => setManaging("settings")} onOpenTasks={onOpenTasks} />;
}
