...
export default function MemberSetup({ catalog, sync, onManage, onRefresh }: {
  ...
  return <article className="keeper-panel member-setup" aria-labelledby="member-setup-heading">
    ...
    {catalog?.jira_connection !== "ready" ? catalog ? <div><strong>Optional: connect Jira</strong><p>Add Jira work if your team uses it. You do not need Jira for Apiary membership.</p><a className="secondary-button" href="#settings-integrations">Connect Jira</a></div> : null
      : <div><strong>{readyProjects.length} Jira {readyProjects.length === 1 ? "project ready" : "projects ready"}</strong>
        {remainingProjects.length > 0 ? <><p>{remainingProjects.length} shared {remainingProjects.length === 1 ? "project has" : "projects have"} additional setup or access requirements. Only configure the projects you use.</p><a className="secondary-button" href="#settings-integrations">Review Jira projects</a></> : null}</div>}
  </article>;
}
