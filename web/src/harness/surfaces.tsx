...
export const SURFACES: Surface[] = [
  ...
  },
  {
    ...
    render: () => <QueuesView workers={demoWorkers} onOpenTask={() => undefined} queenAutomation={{ enabled: true, state: "queued", run_id: "demo", trigger: "actionable_work", actionable_count: 1, attempts: 0, requested_at: now, delivered_at: null, finished_at: null, outcome: null, waiting_reason: "Pacing Queen's next review after a recent delivery" }} tasks={[
      { ...demoTasks[3], id: "scheduled-window", title: "Apply the change in the approved morning window", blocked_until: now + 28_800, blocked_note: "The operator chose tomorrow morning. Other prerequisites must still be rechecked when the window opens." },
      { ...demoTasks[3], id: "mixed-dependencies", title: "Connect the verified contract after access is ready", prerequisites: [
        { task_id: "mixed-dependencies", prerequisite_id: "finished-schema", title: "Verified response schema", state: "completed", assigned_worker_id: null, removed: false, reason: "Response schema is established", created_at: now - 300 },
        { task_id: "mixed-dependencies", prerequisite_id: "pending-access", title: "Provision fixture access", state: "ready", assigned_worker_id: null, removed: false, reason: "Access is still required", created_at: now - 300 },
      ] },
      { ...demoTasks[3], id: "long-blocker-evidence", title: "Recheck the shared contract before resuming", next_move_owner: "queen", blocked_note: "The last check found an incomplete response contract. Queen must verify the current prerequisite and worker evidence before resuming. This historical note is not proof that the dependency remains blocked today." },
      ...
      { ...demoTasks[3], id: "waiting-for-operator", title: "Choose how the search view handles stale results", next_move_owner: "operator", blocked_note: "The contract is ready; the operator needs to choose the fallback behavior.", prerequisites: [{ task_id: "waiting-for-operator", prerequisite_id: demoTasks[2].id, title: "Publish the shared API response contract", state: "completed", assigned_worker_id: demoWorkers[2].id, removed: false, reason: "Contract first", created_at: now - 300 }] },
